...
all specifications (except column 4 in both panels) have a large F statistic on the excluded instrument.
...
\begin{table}[!htp]
    ...
    \makebox[\textwidth][c]{\input{sections/tables/firststage-reg-rawcount.tex}}
    
    \textbf{Panel B: units in log \$ per student}
    
    \makebox[\textwidth][c]{\input{sections/tables/firststage-reg-fte.tex}}
    ...
    $-10$\% state funding shift-share per student in the state leads to $-9.77$\% less state funding per student at the university according to preferred specification column 1.        
...
The results are presented in two different units: Panel A shows how a \$1,000 state funding cut per-student changed the count of faculty, while Panel B shows how a 1\% increase in state funding led to a percent change in the number of faculty per student.
...
\begin{table}[!htp]
    ...
    \makebox[\textwidth][c]{\input{sections/tables/facultycount-shock-reg-rawcount.tex}}
    
    \textbf{Panel B: units in log \$ per student}
    
    \makebox[\textwidth][c]{\input{sections/tables/facultycount-shock-reg-fte.tex}}
    ...
    Panel B shows the effect of a $10$\% change in state funding per student at the university on the 10\% change in the number of professors per student --- i.e.,
    ...
    Standard errors are clustered at the state-year level, and university $+$ year fixed effects are included throughout.
\end{table}
...
\begin{figure}[!htp]
    ...
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        \caption{Lecturers.}
        \includegraphics[width=\textwidth]{sections/figures/lecturer-count-lp.png}
        \label{fig:lecturer-count-lp}
    \end{subfigure}
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        \caption{Assistant Professors.}
        \includegraphics[width=\textwidth]{sections/figures/assistant-count-lp.png}
        \label{fig:assistant-count-lp}
    \end{subfigure}
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        \caption{Full Professors.}
        \includegraphics[width=\textwidth]{sections/figures/full-count-lp.png}
        \label{fig:full-count-lp}
    \end{subfigure}
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        \caption{All Professors.}
        \includegraphics[width=\textwidth]{sections/figures/all-count-lp.png}
...
These effects are a substitution of professors for lecturers at public universities, as the total number of faculty per student are relatively unrelated to funding cuts, but the lecturer count increases and professor count decreases.
This agrees with a model for university departments having fixed teaching requirements making human-resources decisions under constrained resources (e.g., \citealt{abe2015implications}), lending credibility to this type of model for understanding recent human resources decisions of public universities.
...
\begin{table}[!htp]
    ...
    \makebox[\textwidth][c]{\input{sections/tables/facultysalaries-shock-illinois-rolling.tex}}
    
    \makebox[\textwidth][c]{\input{sections/tables/facultyleaving-shock-illinois-rolling.tex}}

    \makebox[\textwidth][c]{\input{sections/tables/promotion-shock-illinois-rolling.tex}}
    ...
    The panel shows the effect of a $1$\% change in state funding per student at the university on faculties' salaries (base salary $+$ benefits), promotion rate (e.g., assistant professor to associate professor), and rate of leaving their university (i.e., by quitting, retiring, or moving to another university).
    Standard errors are clustered at the university-year of hire level, and university $+$ year of hire fixed effects are included throughout.
\end{table}
...
Lecturers receive a 0.75\% increase in salary in the second and third year after a 10\% increase in state funding (\autoref{fig:salaries-lecturer-illinois-lp-rolling}), though they are the only faculty who saw any effect of state funding cuts in terms of salaries.

\begin{figure}[!htp]
    \centering
    \singlespacing
    \caption{Local Projection Estimates for Effect of State Funding on Faculty Salaries at Illinois Public Universities, by Professor Group.}
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        \caption{Lecturers.}
        \includegraphics[width=\textwidth]{sections/figures/salaries-lecturer-illinois-lp-rolling.png}
        \label{fig:salaries-lecturer-illinois-lp-rolling}
    \end{subfigure}
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        \caption{Assistant Professors.}
        \includegraphics[width=\textwidth]{sections/figures/salaries-assistant-illinois-lp-rolling.png}
        \label{fig:salaries-assistant-illinois-lp-rolling}
    \end{subfigure}
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        \caption{Full Professors.}
        \includegraphics[width=\textwidth]{sections/figures/salaries-full-illinois-lp-rolling.png}
        \label{fig:salaries-full-illinois-lp-rolling}
    \end{subfigure}
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        \caption{Administrator Professors.}
        \includegraphics[width=\textwidth]{sections/figures/salaries-administrator-illinois-lp-rolling.png}
...
To study whether these effects hold true nation-wide, I use public data on the number of professors at all US public universities for the entire decade 2011--2021, provided by \citep{wapman2022quantifying},
...
An extra 10\% in state funding is associated with 8.5\% more professors hired per student, as seen in \autoref{fig:hiring-correlation}.
...
\begin{figure}[!h]
    ...
    \includegraphics[width=0.8\textwidth]{sections/figures/hiring-correlation.png}
...
As another robustness check, I employ is a falsification test, testing whether the effects for public universities would be picked up among a sample of private universities; this is a robustness check similar to one performed by \cite{NBERw27885}, as a robustness check for the same instrument.
Private universities do not rely on state funding in the same way that public universities do, so we would expect state funding to not affect their faculty counts.
